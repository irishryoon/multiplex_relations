...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 30 23:11:26 2023
...
def get_rel_distance_vector(data):
    ...
    Macrophage1_coords = np.empty((0,2), int)

    Macrophage1_coords_x = data.loc[(data["celltypes"] == "Macrophage") & (data["phenotypes"] <= 0.5), "points_x"]
    Macrophage1_coords_y = data.loc[(data["celltypes"] == "Macrophage") & (data["phenotypes"] <= 0.5), "points_y"]

    for vertex in range(len(Macrophage1_coords_x)):
         
        Macrophage1_coords = np.append(Macrophage1_coords,[[Macrophage1_coords_x.iloc[vertex],Macrophage1_coords_y.iloc[vertex]]],axis = 0) 


    #Get filtration values

    Macrophage1_filtration_values_vertices, Macrophage1_filtration_values_one_simplices, Macrophage1_filtration_values_two_simplices = getFiltrationValues(Macrophage1_coords, vertices, one_simplices_indices, two_simplices_indices)

    # We feed the filtration values into gudhi and get barcode

    st_Macrophage1, barcode_Macrophage1 = get_rel_barcode(Macrophage1_filtration_values_vertices,Macrophage1_filtration_values_one_simplices,Macrophage1_filtration_values_two_simplices,one_simplices_indices,two_simplices_indices)
    ...
    Macrophage2_coords = np.empty((0,2), int)

    Macrophage2_coords_x = data.loc[(data["celltypes"] == "Macrophage") & (data["phenotypes"] > 0.5), "points_x"]
    Macrophage2_coords_y = data.loc[(data["celltypes"] == "Macrophage") & (data["phenotypes"] > 0.5), "points_y"]

    for vertex in range(len(Macrophage2_coords_x)):
         
        Macrophage2_coords = np.append(Macrophage2_coords,[[Macrophage2_coords_x.iloc[vertex],Macrophage2_coords_y.iloc[vertex]]],axis = 0) 


    #Get filtration values

    Macrophage2_filtration_values_vertices, Macrophage2_filtration_values_one_simplices, Macrophage2_filtration_values_two_simplices = getFiltrationValues(Macrophage2_coords, vertices, one_simplices_indices, two_simplices_indices)

    # We feed the filtration values into gudhi and get barcode

    st_Macrophage2, barcode_Macrophage2 = get_rel_barcode(Macrophage2_filtration_values_vertices,Macrophage2_filtration_values_one_simplices,Macrophage2_filtration_values_two_simplices,one_simplices_indices,two_simplices_indices)
  


    #Create distance vector

    bottleneck_distances = np.empty((12), float)
    ...
    bottleneck_distances[2] = gudhi.bottleneck_distance(st_Tumour.persistence_intervals_in_dimension(0),st_Macrophage1.persistence_intervals_in_dimension(0))
    bottleneck_distances[3] = gudhi.bottleneck_distance(st_Tumour.persistence_intervals_in_dimension(1),st_Macrophage1.persistence_intervals_in_dimension(1))
    bottleneck_distances[4] = gudhi.bottleneck_distance(st_Tumour.persistence_intervals_in_dimension(0),st_Macrophage2.persistence_intervals_in_dimension(0))
    bottleneck_distances[5] = gudhi.bottleneck_distance(st_Tumour.persistence_intervals_in_dimension(1),st_Macrophage2.persistence_intervals_in_dimension(1))
    bottleneck_distances[6] = gudhi.bottleneck_distance(st_Necrotic.persistence_intervals_in_dimension(0),st_Macrophage1.persistence_intervals_in_dimension(0))
    bottleneck_distances[7] = gudhi.bottleneck_distance(st_Necrotic.persistence_intervals_in_dimension(1),st_Macrophage1.persistence_intervals_in_dimension(1))
    bottleneck_distances[8] = gudhi.bottleneck_distance(st_Necrotic.persistence_intervals_in_dimension(0),st_Macrophage2.persistence_intervals_in_dimension(0))
    bottleneck_distances[9] = gudhi.bottleneck_distance(st_Necrotic.persistence_intervals_in_dimension(1),st_Macrophage2.persistence_intervals_in_dimension(1))
    bottleneck_distances[10] = gudhi.bottleneck_distance(st_Macrophage2.persistence_intervals_in_dimension(0),st_Macrophage1.persistence_intervals_in_dimension(0))
    bottleneck_distances[11] = gudhi.bottleneck_distance(st_Macrophage2.persistence_intervals_in_dimension(1),st_Macrophage1.persistence_intervals_in_dimension(1))

    wasserstein_distances = np.empty((12), float)
    ...
    if (len(st_Tumour.persistence_intervals_in_dimension(0)) == 1) & (len(st_Macrophage1.persistence_intervals_in_dimension(0)) == 1):
        wasserstein_distances[2] = abs(st_Tumour.persistence_intervals_in_dimension(0)[0,0] - st_Macrophage1.persistence_intervals_in_dimension(0)[0,0])
    else:
        wasserstein_distances[2] = gudhi.wasserstein.wasserstein_distance(st_Tumour.persistence_intervals_in_dimension(0),st_Macrophage1.persistence_intervals_in_dimension(0), order=1., internal_p=2.)
        
    
    wasserstein_distances[3] = gudhi.wasserstein.wasserstein_distance(st_Tumour.persistence_intervals_in_dimension(1),st_Macrophage1.persistence_intervals_in_dimension(1), order=1., internal_p=2.)
    
    if (len(st_Tumour.persistence_intervals_in_dimension(0)) == 1) & (len(st_Macrophage2.persistence_intervals_in_dimension(0)) == 1):
        wasserstein_distances[4] = abs(st_Tumour.persistence_intervals_in_dimension(0)[0,0] - st_Macrophage2.persistence_intervals_in_dimension(0)[0,0])
    else:
        wasserstein_distances[4] = gudhi.wasserstein.wasserstein_distance(st_Tumour.persistence_intervals_in_dimension(0),st_Macrophage2.persistence_intervals_in_dimension(0), order=1., internal_p=2.)
    
    
    wasserstein_distances[5] = gudhi.wasserstein.wasserstein_distance(st_Tumour.persistence_intervals_in_dimension(1),st_Macrophage2.persistence_intervals_in_dimension(1), order=1., internal_p=2.)
    
    
    if (len(st_Necrotic.persistence_intervals_in_dimension(0)) == 1) & (len(st_Macrophage1.persistence_intervals_in_dimension(0)) == 1):
        wasserstein_distances[6] = abs(st_Necrotic.persistence_intervals_in_dimension(0)[0,0] -  st_Macrophage1.persistence_intervals_in_dimension(0)[0,0] )
    else:
        wasserstein_distances[6] = gudhi.wasserstein.wasserstein_distance(st_Necrotic.persistence_intervals_in_dimension(0),st_Macrophage1.persistence_intervals_in_dimension(0), order=1., internal_p=2.)
    
    wasserstein_distances[7] = gudhi.wasserstein.wasserstein_distance(st_Necrotic.persistence_intervals_in_dimension(1),st_Macrophage1.persistence_intervals_in_dimension(1), order=1., internal_p=2.)
    
    if (len(st_Necrotic.persistence_intervals_in_dimension(0)) == 1) & (len(st_Macrophage2.persistence_intervals_in_dimension(0)) == 1):
        wasserstein_distances[8] = abs(st_Necrotic.persistence_intervals_in_dimension(0)[0,0] - st_Macrophage2.persistence_intervals_in_dimension(0)[0,0])
    else:
        wasserstein_distances[8] = gudhi.wasserstein.wasserstein_distance(st_Necrotic.persistence_intervals_in_dimension(0),st_Macrophage2.persistence_intervals_in_dimension(0), order=1., internal_p=2.)
    
    wasserstein_distances[9] = gudhi.wasserstein.wasserstein_distance(st_Necrotic.persistence_intervals_in_dimension(1),st_Macrophage2.persistence_intervals_in_dimension(1), order=1., internal_p=2.)
    
    if len(st_Macrophage1.persistence_intervals_in_dimension(0)) == 1 & (len(st_Macrophage2.persistence_intervals_in_dimension(0)) == 1):
        wasserstein_distances[10] = abs(st_Macrophage1.persistence_intervals_in_dimension(0)[0,0]- st_Macrophage2.persistence_intervals_in_dimension(0)[0,0])
    else: 
        wasserstein_distances[10] = gudhi.wasserstein.wasserstein_distance(st_Macrophage1.persistence_intervals_in_dimension(0),st_Macrophage2.persistence_intervals_in_dimension(0), order=1., internal_p=2.)
    
    
    wasserstein_distances[11] = gudhi.wasserstein.wasserstein_distance(st_Macrophage1.persistence_intervals_in_dimension(1),st_Macrophage2.persistence_intervals_in_dimension(1), order=1., internal_p=2.)       
...
all_bottleneck_distance_vectors = np.empty((0,12),float)
all_wasserstein_distance_vectors = np.empty((0,12),float)
both_distance_vectors = np.empty((0,24),float)
...
#mapping= {0: ("white", "o"), 1: ("yellow", "o"), 2: ("red","o"),3: ("blue","o")}
...
mapping= {0: ("red", "o"), 1: ("yellow", "o"), 2: ("blue","o")}
...
plt.scatter(show_purity[:,0], show_purity[:,1], s=400, c=show_purity[:,2], marker="o", vmin=0, vmax=1)
plt.title('Purity Bottleneck + Wasserstein')
plt.xlabel('Csf1')
plt.ylabel('chi')
plt.colorbar()
plt.show()
